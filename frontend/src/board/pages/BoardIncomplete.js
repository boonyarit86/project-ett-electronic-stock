...
function BoardIncomplete() {
    ...
    const { isLoading, isLoadingActions, lists, errorMsg, errorMsgActions } = getIncompleteBoard;
                        ...
                        <div className="cover-incomplete" key={item._id}>
                            <div className="header-incomplete">
                                <div className="proflie-img-incomplete">
                                    <img src={item.user.avartar ? item.user.avartar.url : "/images/profile.png"} alt="555" />
                                </div>
                                <div>
                                    <p>{item.user.name} ({item.user.status})</p>
                                    <p>{formatDate(item.hisb.date)}</p>
                                </div>
                            </div>
                            <div className="content-incomplete">
                                <h3>ชื่อบอร์ด {item.board.boardName}</h3>
                                <p><b>รหัสบอร์ด</b> {item.board.boardCode}</p>
                                        ...
                                        <tbody>
                                            {item.tools.map((tool, index) => (
                                                <tr key={tool._id}>
                                                    ...
                                                    <th>{tool.tool.category === "" ? "ไม่ได้กำหนด" : tool.tool.category}</th>
                                                    {/* <th>{tool.size}</th> */}
