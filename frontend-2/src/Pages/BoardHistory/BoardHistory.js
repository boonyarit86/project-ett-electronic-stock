...
import {
  setBoardHistory,
  updateBoardHistory,
} from "../../Redux/features/boardHistorySlice";
...
const BoardHistory = () => {
  ...
  const { boardHistories } = useSelector((state) => state.boardHistory);
  ...
  const [boardTableElement] = useState([
    ...
    { minW: "18", maxW: "20", name: "ชื่อบอร์ด" },
    ...
    if (!controller) {
      ...
      async function fetchData() {
        ...
        await Axios.get(`${process.env.REACT_APP_BACKEND_URL}/boards/history`, {
          ...
          .then((res) => {
            dispatch(endLoading());
            dispatch(setBoardHistory(res.data.data.boardHistories));
            setIsLoading(false);
            setBhList(res.data.data.boardHistories);
            setBhFilter(res.data.data.boardHistories);
          })
          .catch((error) => {
            setIsLoading(false);
            dispatch(endLoading());
  ...
  useEffect(() => {
    if (boardHistories) {
      calcDuration(boardHistories, duration, setBhList);
    }
  }, [duration, boardHistories]);
  ...
  return (
    <div className="boardHistory">
      <Heading type="main" text="ประวัติรายการบอร์ด" className="u-mg-b" />
      {errorMessage && (
        <Toast
          element="error"
          type="default"
