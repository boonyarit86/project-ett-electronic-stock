...
const getAllTools = async (req, res) => {
  try {
    ...
    for(var r=0; r < toolLists.length; r++) {
        await stt.map((item) => {
            
            if(item._id.toString() === toolLists[r].type) {
                toolLists[r].type = item.type
                console.log(toolLists[r])
            } 

            if(toolLists[r].category !== "") {
                for(var x=0; x < item.categorys.length; x++) {
                    if(item.categorys[x]._id.toString() === toolLists[r].category) {
                        toolLists[r].category = item.categorys[x].category
                    } 
                }
            }
        })
...
const actionTool = async (req, res, next) => {
  let tool;
  let cnt;
  const { total, actionType, tid, toolName, description, userName, status } =
    req.body;
  try {
    tool = await Tool.findById(req.params.tid);
  } catch (err) {
